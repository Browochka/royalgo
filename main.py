...
def initialize_particles():
    ...
    ax.set_ylabel('y-coordinate', labelpad=-3)
...
def run_pso():
    ...
    for _ in range(num_iterations):
        ...
        for i in range(len(particles)):
            ...
            velocities[i] = compression_factor * (
                        inertia + cognitive + social) if is_modified_mode else inertia + cognitive + social
...
objective_function_entry.insert(0, "4  (x - 5)² + (y - 6)²")
...
tk.Button(control_frame, text="Инициализировать частицы", command=initialize_particles).grid(row=6, column=0,
